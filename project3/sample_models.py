...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for layer in range(recur_layers):
        rnn_layer = GRU(units, activation = "relu", return_sequences = True, 
                        implementation = 2, name = 'rnn_' + str(layer))(prev_layer)
        bn_rnn = BatchNormalization(name = 'bn_rnn_' + str(layer))(rnn_layer)
        prev_layer = bn_rnn
...
def final_model(input_dim, filters, kernel_size, conv_stride,
    ...
    bidir_rnn = Bidirectional(GRU(units, activation = "relu", 
                                  ...
                                  implementation = 2, 
                                  name = "bidirectional_rnn"))(bn_cnn)
    dropout_rnn = Dropout(rate = 0.2)(bidir_rnn)
    time_dense = TimeDistributed(Dense(output_dim))(dropout_rnn)
    # dropout = Dropout(rate = 0.2)(time_dense)
    y_pred = Activation('softmax', name='softmax')(time_dense)
